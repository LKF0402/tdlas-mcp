...
**背景扣除**：两条归一化都先用「无吸收参考谱」（$\tau\equiv1$，含相同 RAM/AM 与慢漂移、无白/粉红噪声）走同一条链路算出 $S_{1f}^{bg},S_{2f}^{bg}$，再**复减**。它消除的是 L-I 二阶非线性产生的 RAM 基线（非吸收区 2f 残余），只能靠背景扣除去除，降噪 / 提采样率都压不掉。背景扣除可开关（`background_subtract`，**默认 `False`**）——默认出的是**未经修正的原始谱**（保留 L-I 非线性与 RAM 的物理基线），仅供诊断；做浓度反演或检测限结论时须显式设 `background_subtract=true`（此时返回 `normalization.background_subtracted=true`，并在 warnings 与披露项 ⑨ 中说明已扣除）。
