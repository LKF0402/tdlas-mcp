...
### 第十二轮：多浓度锁定归一化方法（`norm_lock`）
- **发现**：把噪声加到 `rin=2e-6` 后，10/20 ppm 判出 `2f/I0`、30 ppm 判出 `2f/1f`——两条曲线量级差两个数量级（0.0036 vs 0.38），叠加图直接作废。根因：`2f/1f` 可用性判据是「非吸收区泄漏 ≤30% 峰值」，噪声下该比值会随浓度翻转。
- **修法**：`wms_normalize` 新增 `norm_lock`（`"2f/1f"` / `"2f/I0"`），调用方可强制方法、跳过自动判定；引擎读 `cfg["norm_lock"]`；MCP 多浓度时用基准浓度判出的方法锁定所有浓度（用户显式给 `norm_lock` 时不介入）。
- **披露**：`multi_conc.normalization_locked` + warnings 一条。
- 与第十一轮的调制锁定同源：**凡是影响纵轴量级的量，多浓度都必须锁定**。
...
- **自检同步**：默认应为未扣除，且显式开启后必须真的生效（两条断言）。
- 文档：`TECHNICAL.md` §背景扣除、`docs/VALIDATION.md` 判据与陷阱表、保真度台账口径同步。

### 第九轮：标准六子图支持多浓度同图
- **主图即多浓度叠加**：`x_list` + `save_png=true` 时，标准 3×2 六子图的 ②~⑤ 面板（PD 原始 / DAS αL / 1f / 2f / 归一化 2f）按 viridis 配色叠加**全部浓度**曲线，图例标 ppm；① 驱动电压各浓度共用，仍画单条；αL 面板额外画一条基准浓度的 HITRAN 理论参考。
- **纵轴按全部浓度统一**：逐条 `seg()` 会把 ylim 不断覆盖成最后一条曲线的范围（低浓度被压成平线）——新增 `_ylim_multi()` 汇总全部浓度的有效区后再定幅，保证各浓度的相对大小真实可比。
- **向后兼容**：单浓度路径完全不变（配色/图例/标题照旧）；`png_overlay` 附加图保留，契约自检仍测。
