...
**每条图都必须**：标注技术名、归一化方法、基线处理方式、线型含义；**1f 与 2f 不得同格**；剔除区（trim_frac 默认 12%）标红带，**纵轴范围只按有效区确定（剔除区不参与）**。

**★ 出图硬约束（已踩坑沉淀，不得违反）**：

1. **面板序号必须完整**：① 波长调制 → ② DAS（归一化透过率）→ ③ DAS 吸光度 vs 理论 → ④ 1f → ⑤ 2f → ⑥ 归一化 2f/1f
2. **DAS 面板画归一化透过率 T/T0**（除以无吸收参考），**不是原始 PD 电压**——原始信号有 L-I 基线斜率，吸收峰被淹没
