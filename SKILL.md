...
| "多浓度对比 / 标定曲线" | `x_list=[...]` + `save_png=true` → **主图 ②~⑤ 面板即按 ppm 叠加**（纵轴按全部浓度统一）**且调制系数锁定**（自适应只在基准浓度跑一次，保证各浓度幅值可比）+ 附加 `png_overlay` 单用途对照 |
...
**每条图都必须**：标注技术名、归一化方法、基线处理方式、线型含义；**1f 与 2f 不得同格**；剔除区（trim_frac 默认 12%）标红带，**纵轴范围只按有效区确定（剔除区不参与）**。**剔除区只对 WMS 面板（1f/2f/归一化）有效，DAS 面板不画剔除区**（DAS 无锁相瞬态）。

**★ 出图硬约束（已踩坑沉淀，不得违反）**：

1. **面板序号必须完整**：① 波长调制 V(t) → ② DAS 原始信号 → ③ DAS 吸光度 vs 理论 → ④ 1f → ⑤ 2f → ⑥ 归一化 2f/1f
2. **DAS 面板按模式区分**：单浓度画**原始 PD 电压**（探测器采集到什么样就是什么样，斜的 L-I 曲线，不扣基线）；多浓度（x_list）画**归一化透过率 T/T0**（除以有效区中位数），便于各浓度叠加对比
3. **所有谱图纵轴只按有效区数据定**：剔除区（边缘瞬态）的高值**不得**参与纵轴范围计算，否则有效区峰形被压扁
4. **多浓度叠加纵轴**：按所有浓度的有效区统一范围，保证各浓度相对大小可比
5. **剔除区画法**：灰虚线 + 红色背景带，图例标注"转折点，不可信"；剔除区数据不用于定峰高、不定纵轴
...
**`offer_device_packing` 出现时**（`severity=advisory`，不阻断结论）**必须向用户转述**，并按其中的 `missing_devices` 逐类索取型号。它携带 `{"reason", "missing_devices": [{"cn","fields":[{"cn","unit"}]}], "how"}`。**该提示同一会话只下发一次**，看到就处理。服务器何时提示：设备库空 → 主动提示建库（告诉用户当前是演示默认值，引导录入自己的硬件）；有设备无 setup → 提示打包；有 setup 无默认 → 提示设默认；都配好 → 静默。
